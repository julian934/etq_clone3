...
import { getPoloShirts } from '@/app/lib/database/connections'
...
export const metadata:Metadata={
  title:'Polo Shirts',
  description:'The Polo Shirts Product Page',
  creator:'Julian Borner'
}
type Props = {}

const PoloShirts = (props: Props) => {
  const [filterModal,setFilterModal]=useState<any>(null);
  const {data}=useQuery({
    queryKey:['poloShirts'],
    queryFn:()=>getPoloShirts(),
    staleTime:1000*60*5
  })
    ...
    <div className='flex min-md:w-1/5  w-full justify-between px-4 space-x-2 ' >
      ...
      <Link className='self-center' href='/collections/menswear' >Menswear</Link>
       <hr className=' h-6 origin-center  self-center rotate-0 border-2 ' />
        <h1 className='self-center w-full  ' >Polo Shirts</h1>
    </div>
