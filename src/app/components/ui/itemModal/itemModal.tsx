  ...
  return (
        <motion.div className=' w-64 h-64 relative '  onHoverStart={()=>setModalActive(!modalActive)} onHoverEnd={()=> modalActive && setModalActive(!modalActive)} >
            ...
            {modalActive &&   <motion.div className='relative z-[9999]' >
                <motion.button className='relative -top-36  rounded-2xl ' onClick={()=>indLeft()} onHoverStart={()=>!leftHover && setLeftHover(!leftHover)}  onHoverEnd={()=>leftHover && setLeftHover(!leftHover)}>{leftHover?<LongLeftArrow/>:<LeftArrow/>}</motion.button> 
